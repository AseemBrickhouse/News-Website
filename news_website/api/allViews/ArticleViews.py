...
class AllArticles(APIView):
    def get(self, request, *args, **kwargs):
        def get_popular_articles(user_account, display_count):
            popQuerySetRequest = Article.objects.all().filter(
                isPrivate=False).order_by('rating').reverse()[:display_count]
            popqueryset = {}
            for article in popQuerySetRequest:
                articleJson = ArticleSerializer(article).data
                key = articleJson['key']
                reporter_account = Account.objects.all().filter(
                    id=articleJson['reporter_account'])[0]
                articleJson['reporter_account'] = AccountSerializer(
                    reporter_account).data
                articleJson['reporter_account']['followers'] = getFollow(
        ...
        for article in queryset:
            ...
            reporter_account = Account.objects.all().filter(
                id=articleJson.data['reporter_account'])[0]
            querysetSend[key]['reporter_account'] = AccountSerializer(
                reporter_account).data
            querysetSend[key]['reporter_account']['followers'] = getFollow(
                reporter_account, "FOLLOWERS")
...
class GetArticle(ObtainAuthToken):
    def get(self, request, *args, **kwargs):
        ...
        reporter_account = Account.objects.get(
            id=articleJson['reporter_account'])
        articleJson['reporter_account'] = AccountSerializer(
            reporter_account).data
...
class AllUserArticles(ObtainAuthToken):
    def get(self, request, *args, **kwargs):
        ...
        for article in queryset:
            ...
            AllArticles[article_json.data['id']
                        ]['reporter_account'] = attachNameToArticle(article_json)
...
class GetUserArticles(APIView):
    def get(self, request, *args, **kwargs):
        ...
        for article in userArticles:
            ...
            articlesJson['reporter_account'] = AccountSerializer(
                reporter_account).data
            articlesJson['reporter_account']['followers'] = getFollow(
                reporter_account, "FOLLOWERS")
            if user_account != None:
                try:
                    BookmarkedArticles.objects.get(
...
class PopularArticles(APIView):
    def get(self, request, *args, **kwargs):
        ...
        queryset = Article.objects.all().filter(
            isPrivate=False).order_by('rating').reverse()[:3]
...
class MyBookmarkedArticles(ObtainAuthToken):
    def get(self, request, *args, **kwargs):
        ...
        for entry in bookmarked:
            queryset[ArticleSerializer(entry.saved).data['key']] = ArticleSerializer(
                entry.saved).data
