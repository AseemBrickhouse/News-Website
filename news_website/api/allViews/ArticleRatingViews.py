...
class ArticleRatingView(ObtainAuthToken, APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        article_rating_data = {
            ...
            'current_article_rating_vote': None

        }

        try:
            rating = ArticleRating.objects.get(
                article=query_article, account=user_account)
            # Doesn't throw an error so it exists in DB
            article_rating_data['current_article_rating_vote'] = rating.vote_type
    ...
    def delete(self, request, *args, **kwargs):
        ...
        deleted_article_rating = ArticleRatingSerializer(
            query_article_rating).data
